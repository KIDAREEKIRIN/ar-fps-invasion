...
const ENEMY_TYPES = {
  scout:  { hp: 1, speed: 3.2, speedCap: 5.0, scale: 0.65, geo: "scout",  color: 0x2bf5c8, emissive: 0x0a4a3c, damage: 8,  score: 120, ranged: false },
  drone:  { hp: 1, speed: 1.9, speedCap: 4.2, scale: 1.0,  geo: "drone",  color: 0xff3b5c, emissive: 0x550010, damage: 14, score: 100, ranged: false },
  brute:  { hp: 3, speed: 1.1, speedCap: 2.6, scale: 1.6,  geo: "brute",  color: 0xff7a1a, emissive: 0x5a2400, damage: 26, score: 280, ranged: false },
  turret: { hp: 2, speed: 1.4, speedCap: 2.4, scale: 1.05, geo: "turret", color: 0xb14dff, emissive: 0x3a0a5a, damage: 18, score: 220, ranged: true, range: 7.5, fireCD: 2.0, projDamage: 12, maxShots: 6 },
};
...
const game = {
  ...
  ammo: 12, magSize: 12, reloading: false, reloadStart: 0, reloadDur: 800,
...
const sfx = {
  shoot() { noise({ dur: 0.07, vol: 0.16 }); blip({ type: "square", freq: 220, dur: 0.06, vol: 0.1, slide: -120 }); },
  hit() { blip({ type: "sine", freq: 880, dur: 0.05, vol: 0.16, slide: 300 }); },
  crit() { blip({ type: "sine", freq: 1200, dur: 0.09, vol: 0.2, slide: 500 }); },
...
function spawnEnemy(typeKey) {
  ...
  en.group.scale.setScalar(cfg.scale);
...
function fire() {
  ...
  if (hits.length > 0) {
    ...
    if (obj.userData.kind === "proj") {
      ...
      if (p && p.alive) { spawnBurst(p.mesh.position, 0xff5cf0); recycleProjectile(p); addScore(20); showHitmarker(false); }
    } else if (obj.userData.kind === "enemy") {
      const en = obj.userData.enemy;
      if (en && en.alive) hitEnemy(en);
    }
  }
  if (game.ammo <= 0 && game.overdrive <= 0) setTimeout(reload, 300);
}
...
function hitEnemy(en) {
  ...
  if (en.hp > 0) {
    showHitmarker(false);
    if (crit) showPopup("치명타!", s.x, s.y, "crit");
    // 피격 반응
    en.body.material.emissiveIntensity = 2;
  } else {
    destroyEnemy(en, crit);
  }
}

function destroyEnemy(en, crit) {
  const pos = en.group.position;
  spawnBurst(pos, en.cfg.color);
  sfx.explode(); vibrate(crit ? 30 : 18); addShake(crit ? 8 : 5);
  showHitmarker(true);
  ...
  const gained = Math.round(en.cfg.score * (crit ? 2 : 1) * mult);
  addScore(gained);
  updateComboUI();

  const s = worldToScreen(pos);
  showPopup((crit ? "치명타 " : "") + "+" + gained, s.x, s.y, crit ? "crit" : "");

  // 파워업 드롭 (12%)
  if (Math.random() < 0.12) spawnPickup(pos.clone());
...
function reload() {
  ...
  game.reloading = true;
  game.reloadStart = performance.now();
  updateAmmo();
...
function pickEnemyType() {
  const w = game.wave;
  const r = Math.random();
  if (w >= 3 && r < 0.18) return "turret";
  if (w >= 2 && r < 0.40) return "brute";
  if (r < 0.65) return "scout";
  return "drone";
}
...
function updateWaves(dt) {
  if (game.betweenWaves) {
    ...
    if (game.waveBreakTimer <= 0) startWave(game.wave + 1);
    return;
  }
  // 동시 존재 적 수 상한 (후반 화면 도배·불공정 방지)
  const maxAlive = Math.min(5 + game.wave * 2, 16);
  if (game.enemiesToSpawn > 0) {
    ...
    addScore(50 * game.wave);
    showCenterText("웨이브 클리어!");
  }
}
...
/* =========================================================
   성능 모니터 (적응형 해상도)
   ========================================================= */
const perf = { acc: 0, frames: 0, fps: 60, pr: Math.min(window.devicePixelRatio, 2), lastAdjust: 0 };
...
function animate() {
  ...
  if (game.state === State.PLAYING) {
    ...
    // 재장전 진행바
    if (game.reloading) {
      const p = Math.min(1, (performance.now() - game.reloadStart) / game.reloadDur);
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
      ...
      if (en.cfg.ranged && en.shotsLeft > 0 && dist < en.cfg.range) {
        // 포탑: 사거리에서 정해진 횟수만 사격
        en.attackCD -= dt;
        if (en.attackCD <= 0) { spawnProjectile(en.group.position, en.cfg.projDamage); en.attackCD = en.cfg.fireCD; en.shotsLeft--; }
      ...
      if (en.body.material.emissiveIntensity > 1) en.body.material.emissiveIntensity = Math.max(1, en.body.material.emissiveIntensity - dt * 6);
...
function resetGame() {
  ...
  game.reloading = false; game.betweenWaves = false;
...
function bindUI() {
  ...
  fireBtn.addEventListener("touchstart", (e) => { e.stopPropagation(); e.preventDefault(); fire(); }, { passive: false });
  ...
  document.querySelectorAll(".aim-seg button").forEach((b) => {
    b.addEventListener("click", (e) => { e.stopPropagation(); setAimMode(b.dataset.mode); });
  });
  // 정면 보정 버튼 (인게임 + 일시정지)
